...
define('STYLE_WEB_ROOT', get_stylesheet_uri() );
...
if ( !function_exists( 'lambda_registerstyles' ) ) {
	...
	add_action('get_header', 'lambda_registerstyles');
		...
		function lambda_registerstyles() {
			...
			global $lambda_meta_data, $theme_options, $cookiestatus;
			...
			$stylesheets .= wp_enqueue_style('color', THEME_WEB_ROOT.'/css/colors/'.$themefile, false, UT_THEME_VERSION, 'screen, projection');
			...
			$stylesheets .= wp_enqueue_style('responsive', THEME_WEB_ROOT.'/responsive.css', 'theme', UT_THEME_VERSION, 'screen, projection');
...
if ( !function_exists( 'lambda_header_scripts' ) ) {
	...
	function lambda_header_scripts() {
			...
			if(!is_admin()) {
				...
				//custom javascript
				$javascripts .= wp_enqueue_script('custom',THEME_WEB_ROOT ."/javascripts/app.js",array('jquery'),'1.2.3', true);
...
if ( !function_exists( 'script_exceptions' ) ) {
	...
	function script_exceptions() {
		...
		$script_exceptions  .= wp_enqueue_script('twitter',THEME_WEB_ROOT ."/javascripts/jquery.tweet.js",array('jquery'),'1.0',true); }
...
function lambda_setup() {
	...
	#-----------------------------------------------------------------
	load_theme_textdomain( UT_THEME_NAME, THEME_DOCUMENT_ROOT  . '/languages' );
...
if(lambda_is_plugin_active('woocommerce/woocommerce.php')) {
	...
	include_once(THEME_DOCUMENT_ROOT  . '/woocommerce/widgets/lambda_woo_login.php');
...
$files = @scandir(THEME_DOCUMENT_ROOT."/addons/");
if ( count($files) > 2 ) {
